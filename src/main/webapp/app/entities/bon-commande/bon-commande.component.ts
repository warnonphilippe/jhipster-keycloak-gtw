...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/bon-commande'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: BonCommande) {
        return item.id;
    }
    registerChangeInBonCommandes() {
        this.eventSubscriber = this.eventManager.subscribe('bonCommandeListModification', (response) => this.loadAll());
    }
